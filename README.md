...
## 빌드 및 배포
   ...
   앞으로의 큰 규모의 프로젝트를 고려해서 Jenkins를 이용해 Unity 빌드 자동화 및 배포를 작업했다.
   이를 통해 소스 코드 변경 시 자동으로 빌드를 수행하고 배포하며 결과를 알림 받는 시스템을 구현했다.
   
  크게 3가지 상태 빌드 전, 빌드 진행, 빌드 후 로 나눌 수 있다
  
  #빌드 전
   - Jenkins가 github 의 변경 점을 트리거로 간주 하고 빌드를 준비한다.
     
  #빌드 진행
   - build 스크립트를 실행 하여 프로젝트를 빌드 하고 지정한 로컬 주소로 배포한다.
     
  #빌드 후
   - 이 프로젝트의 경우 빌드 시간이 20 ~ 25 초로 짧았지만 더 큰 규모의 프로젝트를 대비해 알람 기능을 설정했다.
   - 처음엔 이메일 알람을 생각했고 트리거를 always 로 설정하여 항상 빌드 결과를 볼 수 있도록 했다.
   - gmail은 새로운 보안 정책에 의해 메일을 받을 수 없었고 naver 메일은 보안 설정을 변경해도 Jenkins 에서 전송항 메일을 받을 수 없었다.
   - 보안 문제를 해결하기 위해 접근성이 좋은 discord로 알람을 받기로 했고 webhook 방식으로 알람을 성공적으로 받았다.
   - 
   
   ## 5. 향후 개선 사항
   - 자동화된 빌드 및 배포 시스템을 지속적으로 개선하여 **CI/CD** 파이프라인을 확장할 계획입니다.
   - 테스트 자동화 및 서버 최적화 작업도 함께 진행할 예정입니다.
